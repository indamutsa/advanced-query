...
class Parser {
    ...
    checkIfValidToken() {
        if (this.token_index === 0) {
            if (!['TT_SPACE', 'TT_LBRAKET', 'TT_RBRAKET', 'TT_KEYWORD', 'TT_NUMBER', 'TT_TAG'].includes(this.currentToken.type)) {
                return new InvalidToken(this.currentToken, this.token_index, "Invalid starting token!");
            }
        }

        if ((['TT_AND', 'TT_OR', 'TT_NOT'].includes(this.tokens[this.tokens.length - 2].type))) {
            return new InvalidToken(this.tokens[this.tokens.length - 2], this.token_index, "Invalid ending token!");
        }
    }
    ...
    parse() {
        ...
        if (this.currentToken.type !== 'TT_EOF')
            return {
                result: null, error: new InvalidToken(
                    this.currentToken, this.token_index, "Invalid ending or misplaced token!")
            };
    ...
    term() {
        ...
        if (isInstanceOf(leftNode, InvalidToken)) return { result: null, error: leftNode }

        while (this.operators.includes(this.currentToken.type)) {
            if (this.metricsOp.includes(this.currentToken.type) && this.tokens[this.token_index + 1].type !== 'TT_NUMBER') {
                return new InvalidToken(this.currentToken, this.token_index, "Conditional statement can be followed by a number!");
            }
            let operator_token = this.currentToken;
            this.advance();

            let rightNode = this.factor();
            if (isInstanceOf(rightNode, InvalidToken)) return { result: null, error: rightNode }
    ...
    factor() {
        ...
        // Return a tag node, it extracts the tag, colon and the value
            ...
            if (this.currentToken.type === 'TT_COLON') {
                ...
                this.advance();
            } else return new InvalidToken(this.currentToken, this.token_index, "Expected colon!");

            let value = this.getTagValue();
            if (isInstanceOf(value, InvalidToken)) return { result: null, error: value }
        ...
        // Return a metric node, it extracts the metric, operator and the value
            ...
            if (this.metricsOp.includes(this.currentToken.type)) {
                ...
                else return new InvalidToken(this.currentToken, this.token_index, "Expected number!");
            } else return new InvalidToken(this.currentToken, this.token_index, "Expected conditional operator!");
        ...
        // Return the exact match when defined, the string is enclosed in single quotes
            ...
            return new InvalidToken(this.currentToken, this.token_index, "Misplaced Token, please chech the syntax!");
    }
    ...
    getTagValue() {
        ...
        if (token.type === 'TT_NUMBER' || token.type === 'TT_KEYWORD') {
            ...
            let endingQuote = "'";
            this.advance();
            return { result: new ExactKeywordNode(startingQuote, str, endingQuote), error: null };
        } else if (token.type === 'TT_LBRAKET') {
            this.advance();
            let node = this.term();
            if (isInstanceOf(node, InvalidToken)) return { result: null, error: node }
...
module.exports = { Parser, BinaryOperatorNode, Node, TagNode, MetricNode, ExactKeywordNode };