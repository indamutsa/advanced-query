...
class Lexer {
      ...
      switch (this.currentChar) {
        ...
        case "A":
          if (this.checkStringEquality("AND")) {
            ...
            if (keyword instanceof IllegalCharacterError) return keyword;
        ...
        default:
          if (this.currentChar.match(/[0-9]/)) {
            ...
            if (number instanceof IllegalCharacterError) return number;
            ...
            if (tags.includes(keyword)) {
              ...
              if (token?.type === "TT_QUOTE")
                this.tokens.push(new Token("TT_KEYWORD", keyword));
              else this.tokens.push(new Token("TT_TAG", keyword));
            } else if (metrics.includes(keyword))
              this.tokens.push(new Token("TT_METRIC", keyword));
            else this.tokens.push(new Token("TT_KEYWORD", keyword));
            // this.advance();
          } else {
            return new Token(
              "TT_ERROR",
      ...
      if (this.tokens[i].type === "TT_SPACE") {
        if (
          [
            ...
            "TT_GREATER_THAN",
            "TT_LESS_THAN",
          ].includes(this.tokens[i + 1].type)
        ) {
          this.tokens.splice(i, 1);
          continue;
        } else if (
          this.tokens[i + 1].type === "TT_RBRAKET" ||
          this.tokens[i + 1].type === "TT_EOF"
        ) {
          this.tokens.splice(i, 1);
          continue;
  ...
  makeNumberToken() {
    ...
    if (
      (dot_count === 0 || dot_count === 1) &&
      [" ", "]", null].includes(this.currentChar)
    )
      return number;
    else
      return new IllegalCharacterError(
